...
async def show_ads_category(
    ...
    return await ViewModel(
        ...
        all_ads = all_ads[:9],
        all_cities = await ad_service.get_cities_by_category(
            category_id, 
            session
        ),
        all_subcategories = (
            await category_service.get_subcategory_by_category_id(
                category_id, 
                session
            )
        ),
...
async def show_ads_category(
    ...
    return await ViewModel(
        all_categories = await category_service.get_all_categories(session),
        all_ads = all_ads[:9],
        all_cities = await ad_service.get_cities_by_subcategory(
            subcategory_id, 
            session
        ),
        all_subcategories = [],
...
async def search_by_title(
    ...
    ads_found, total_ads_found = (
        await ad_service.get_ads_by_title_or_description(
            session, 
            title,
            description
        )
    ...
    return await ViewModel(
        all_categories = await category_service.get_all_categories(session),
        all_ads = ads_found[:9],
        all_cities = await ad_service.get_cities_with_ads_by_text(
            title, 
            session
        ),
        all_subcategories = [],
...
async def post_ad_viewmodel(
    ...
    vm = await ViewModel(
        ...
        is_negotiable = (
            form_field_as_str(form_dict, 'negotiable')
            if 'negotiable' in form_dict else None
        ),
    ...
    if not is_valid_txt_field(vm.title):
        vm.error, vm.error_msg = True, """Título inválido. 
            Por favor, evite utilizar símbolos mais de três vezes seguidas.
        """
        ...
        vm.error, vm.error_msg = True, """
            Insira um número de telemóvel com o seguinte formato:
            +351 123 456 789 ou 123 456 789
        """
        ...
        vm.error, vm.error_msg = True, """
            O anúncio deve ter pelo menos duas 2 imagens.
        """
    elif len(files) >= 2:
        for file in files:
    ...
    if not vm.error:
        ...
        await ad_service.insert_ad(
            ...
            ad_status_id = 2,
            authenticity = (
                'Não original' if vm.authenticity == '0' 
                else 'Original'
            ),
            session = session
        )
...
async def upgrade_ad_promo(
    ...
    promo = await ad_service.get_promo_by_id(
        checkout_data.promo_id, 
        db_session
    )
    session = get_session()
    ...
    user_data = await user_service.get_user_login_data_by_id(
        user.user_id, 
        db_session
    )
    try:
