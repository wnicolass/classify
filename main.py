...
def start_uvicorn(
        create_ddl: bool = typer.Option(
            False, 
            "--create-ddl", 
            "-c", 
            help = "Drop and Create all tables. [default: False]"
        ),
        create_dml: bool = typer.Option(
            False, 
            '--crete-dml', 
            '-d', 
            help = "Insert initil data into database."
        ),
        auto_reload: bool = typer.Option(
            ...
            "-r", 
            help = """
                    Define if the app will reload after each saving. 
                    [default: False]
                """
            ),
        host: str = typer.Argument(
            "127.0.0.1", 
            help = "IP Address in which the server will be executed."
        ),
        port: int = typer.Option(
            8000, 
            "--port", 
            "-p", 
            help = "Port in which the server will listen."
        )
    ):
    """
...
def config_exception_handlers():
    ...
    app.add_exception_handler(
        HTTPUnauthenticatedOnly, 
        unauthenticated_only_area_handler
    )
